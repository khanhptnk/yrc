...
class InnerDeepSVDD(nn.Module):
    ...
    def _build_model(self):
        ...
        layers.add_module(
            "input_layer",
            nn.Linear(self.n_features, self.hidden_neurons[0], bias=False),
        )
        layers.add_module(
        ...
        if self.use_ae:
            ...
            layers.add_module(
                f"output_activation", get_activation_by_name(self.output_activation)
            )
        return layers
            ...
            for batch_x, _ in dataloader:
                ...
                if self.use_ae:
                    loss = (
                        torch.mean(dist)
                        # + w_d
                        + torch.mean(torch.square(outputs - batch_x))
                    )
                else:
