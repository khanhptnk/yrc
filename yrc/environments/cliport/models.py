...
from lib.cliport.cliport.agents.transporter_lang_goal import (
...
class CliportModel(TwoStreamClipLingUNetLatTransporterAgent):
    def __init__(self, name, cfg):
        ...
        dummy_pick_features, dummy_place_features = self.extract_features(
            dummy_image, dummy_info
        )
        self.hidden_dim = np.concatenate(
            (dummy_pick_features[0], dummy_place_features[0]), axis=0
        ).shape[0]
        self.logit_dim = self.get_logit(
            {"image": dummy_image, "info": dummy_info[0]}
        ).shape[1]
    ...
    def get_hidden(self, obs):
        ...
        pick_hidden = (
            torch.stack(pick_hidden) if isinstance(pick_hidden, list) else pick_hidden
        )
        place_hidden = (
            torch.stack(place_hidden)
            if isinstance(place_hidden, list)
            else place_hidden
        )
    ...
    def get_logit(self, obs):
        ...
        return torch.cat(
            (
                attention_logits.flatten().unsqueeze(0),
                transport_logits.flatten().unsqueeze(0),
            ),
            dim=-1,
        )
