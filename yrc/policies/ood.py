...
class OODPolicyConfig:
    ...
    epoch: int = 100
    batch_size: int = 32
    ood_detector: str = "DeepSVDD"
    collect_data_agent: str = "novice"
...
class OODPolicy(Policy):
    ...
    def maybe_convert_to_tensor(self, features):
        ...
        if isinstance(
            ...
            return [
                self.to_tensor(f) if not torch.is_tensor(f) else f for f in features
            ]
    ...
    def _rollout_once(self, env):
        ...
        def get_features(obs, feature_type):
            ...
            feature_map = {
                ...
                if get_global_variable("benchmark") == "cliport"
                else [obs["env_obs"], obs["novice_features"]],
                "dist": lambda obs: obs["novice_logit"],
                "hidden_dist": lambda obs: [
                    obs["novice_features"],
                    obs["novice_logit"],
                ],
                "obs_dist": lambda obs: [obs["env_obs"]["image"], obs["novice_logit"]]
                if get_global_variable("benchmark") == "cliport"
                else [obs["env_obs"], obs["novice_logit"]],
                ...
                if get_global_variable("benchmark") == "cliport"
                else [obs["env_obs"], obs["novice_features"], obs["novice_logit"]],
            }
        ...
        if self.args.method == "DeepSVDD":
            ...
            feature_type_to_shapes = {
                ...
                "hidden_obs": lambda dummy_obs: (
                    ...
                    + dummy_obs["novice_features"].shape[1:]
                ),
                "dist": lambda dummy_obs: dummy_obs["novice_logit"].shape,
                "hidden_dist": lambda dummy_obs: (
                    dummy_obs["novice_features"].shape
                    + dummy_obs["novice_logit"].shape[1:]
                ),
